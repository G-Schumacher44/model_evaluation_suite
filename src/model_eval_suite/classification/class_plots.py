...
def _get_run_save_dir(config: SuiteConfig, sub_dir: str | None = None) -> Path:
...
# --- SHAP PLOT FUNCTIONS ---
def plot_shap_summary_beeswarm(shap_explanation: Explanation, config: SuiteConfig) -> str | None:
...
def plot_shap_bar(shap_explanation: Explanation, config: SuiteConfig) -> str | None:
...
def plot_feature_distributions(
    X: pd.DataFrame, y: pd.Series, config: SuiteConfig
) -> dict[str, str]:
    dist_plot_paths = {}
    ...
    for col in numeric_features:
        ...
        ax.hist(
            [class_0_data, class_1_data],
            bins="auto",
            stacked=True,
            label=["Stay (Class 0)", "Churn (Class 1)"],
            edgecolor="white",
        )
...
def plot_confusion_matrix(
    model: Pipeline, X_test: pd.DataFrame, y_test: pd.Series, config: SuiteConfig
) -> str | None:
    save_dir = _get_run_save_dir(config, "evaluation")
...
def plot_learning_curve(
    model: Pipeline, X_train: pd.DataFrame, y_train: pd.Series, config: SuiteConfig
) -> str | None:
    ...
    ax.set_title(f"Learning Curve - {config.run_id}")
...
def plot_calibration_curve(
    ...
    ax.set_title(f"Calibration Curve - {config.run_id}")
    ax.legend()
    ax.grid(True)
    plt.savefig(save_path, bbox_inches="tight")
    plt.close(fig)
...
def plot_threshold_analysis(
    ...
    ax.plot(thresholds, f1_scores[:-1], label="F1-Score", lw=2)
    ax.set_title(f"Threshold Analysis - {config.run_id}")
...
def plot_permutation_importance(
    ...
    result = permutation_importance(
        model, X_test, y_test, n_repeats=10, random_state=42, n_jobs=-1, scoring="f1"
    )
...
def plot_cumulative_gain(
    ...
    fig, ax = plt.subplots()
    ax.plot(percentages, gains, label="Model")
    ax.plot([0, 1], [0, 1], "k--", label="Random")
    ax.set_xlabel("Percentage of sample")
    ax.set_ylabel("Gain (Percentage of positives captured)")
    ax.set_title(f"Cumulative Gains Curve - {config.run_id}")
    ax.legend()
    ax.grid(True)
    plt.savefig(save_path)
    plt.close(fig)
...
def plot_lift_curve(
    ...
    fig, ax = plt.subplots()
    ax.plot(percentages, lift, label="Model")
    ax.plot([0, 1], [1, 1], "k--", label="Baseline")
    ax.set_xlabel("Percentage of sample")
    ax.set_ylabel("Lift")
    ax.set_title(f"Lift Curve - {config.run_id}")
    ax.legend()
    ax.grid(True)
    plt.savefig(save_path)
    plt.close(fig)
...
def plot_feature_coefficients(model: Pipeline, config: SuiteConfig, save_dir: Path) -> str | None:
...
def plot_cv_score_distribution(
    results_data: dict[str, Any], config: SuiteConfig, save_dir: Path
) -> str | None:
    ...
    ax.set_ylim(
        min(cv_scores) - 0.01, max(cv_scores) + 0.01
    )  # Add a small buffer around score range
    ...
    for name, func in plot_functions.items():
        if getattr(plot_configs, name).get("save", False):
            args = (
                ...
                else (model, X_test, y_test, config)
            )
