...
def generate_audit_alerts(results: dict, config: SuiteConfig) -> list:
    ...
    if train_f1 > test_f1 * audit_config.overfitting_threshold_factor:
        alerts.append(
            f"⚠️ Overfitting Signal: Training F1 ({train_f1:.3f}) is significantly higher than Test F1 ({test_f1:.3f})."
        )
    ...
    if cv_std > audit_config.cv_std_threshold:
        alerts.append(
            f"⚠️ High CV Variance: F1 standard deviation across folds is high ({cv_std:.3f})."
        )
    ...
    if baseline:
        ...
        if test_f1 < baseline_f1 * audit_config.performance_regression_f1_threshold_factor:
            alerts.append(
                f"⚠️ Performance Regression: Test F1 ({test_f1:.3f}) is >5% lower than baseline ({baseline_f1:.3f})."
            )
