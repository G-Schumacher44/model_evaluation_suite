...
class EvaluationDashboard:
        ...
        for name, path_str in plot_paths.items():
            ...
            if path.exists() and name in importance_plot_keys:
                ...
                image_widget = widgets.Image(
                    value=image_bytes, format=path.suffix.lstrip("."), width=600
                )
    ...
    def _build_plot_viewer_tab(self, title: str, sub_directory: str | None = None) -> widgets.VBox:
        """Builds a tab content using the PlotViewer widget."""
        if not self.config:
            return widgets.VBox([widgets.HTML("<i>Configuration not available.</i>")])
        ...
        if not plot_dir.is_dir() or not any(
            p.suffix.lower() in PlotViewer.SUPPORTED_FORMATS for p in plot_dir.iterdir()
        ):
            return widgets.VBox(
                [
                    widgets.HTML(
                        f"<p><em>No plot images found in '{sub_directory or 'main plots'}'.</em></p>"
                    )
                ]
            )
    ...
    def _build_baseline_tab(self) -> widgets.HTML:
        ...
        data = [
            {
                "Metric": m.replace("test_", "").replace("_", " ").title(),
    ...
    def _build_explainer_tab(self) -> widgets.VBox:
        ...
        description_widget = widgets.HTML(
            "<p>Enter a row index from the SHAP sample to see its local explanation.</p>"
        )
        index_input = widgets.IntText(
            value=0, description="Row Index:", style={"description_width": "initial"}
        )
                ...
                try:
                    ...
                    with warnings.catch_warnings():
                        warnings.filterwarnings(
                            "ignore", message=".*Javascript library not loaded.*"
                        )
                    ...
                    print(
                        "ℹ️ Interactive SHAP visualization unavailable (static plots available in Evaluation Plots tab)"
                    )

        explain_button.on_click(on_explain_click)

        title_widget = widgets.HTML(
            '<h4 style="margin-top:0; border-bottom: 1px solid #e1e4e8; padding-bottom: 8px;">Local Prediction Explainer</h4>'
        )
        content_widget = widgets.VBox([description_widget, interactive_controls, output_area])
        card_container = widgets.VBox(
            [title_widget, content_widget],
    ...
    def _build_reports_tab(self) -> widgets.VBox:
        ...
        train_report = self.results.get("metrics", {}).get(
            "train_classification_report", "Not available."
        )
        test_report = self.results.get("metrics", {}).get(
            "test_classification_report", "Not available."
        )
        report_widgets.append(
            _build_html_card("Train Classification Report", f"<pre>{train_report}</pre>")
        )
        report_widgets.append(
            _build_html_card("Test Classification Report", f"<pre>{test_report}</pre>")
        )
        ...
        if summary_path_str:
            try:
                ...
                if summary_path.exists():
                    ...
                    summary_card = _build_html_card(
                        "Statistical Summary (statsmodels)", f"<pre>{summary_text}</pre>"
                    )
    ...
    def _build_config_tab(self) -> widgets.HTML:
        ...
        return widgets.HTML(
            f"<pre style='white-space: pre-wrap; word-wrap: break-word;'>{config_str}</pre>"
        )
    ...
    def _build_dashboard(self) -> widgets.VBox:
        ...
        tab_children = [
            ...
            self._build_plot_viewer_tab(
                title="Feature Distribution Plots", sub_directory="distributions"
            ),
