...
from . import class_metrics, class_plots
...
def orchestrate_model_evaluation(
    ...
    if "shap_values" in results and config.evaluation.plots.shap_summary.get("save", False):
        ...
        plot_paths["shap_bar"] = class_plots.plot_shap_bar(results["shap_values"], config)
    ...
    if isinstance(model.named_steps["estimator"], LogisticRegression):
        try:
            ...
            print(
                "⚠️ statsmodels is not installed or has a version conflict. Cannot generate statistical summary."
            )
