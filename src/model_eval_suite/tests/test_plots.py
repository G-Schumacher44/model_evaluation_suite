...
from model_eval_suite.utils.config import (
    EvaluationConfig,
    ModelingConfig,
    PathsConfig,
...
def dummy_config(tmp_path):
    return SuiteConfig(
        ...
        modeling=ModelingConfig(
            target_column="label", pipeline_factory=PipelineFactoryConfig(name="LogisticRegression")
        ),
        evaluation=EvaluationConfig(
            run=True, export_xlsx_summary=False, export_html_dashboard=False, plots=PlotConfig()
        ),
    )
...
def dummy_model(dummy_data):
    X, y = dummy_data
    preprocessor = ColumnTransformer([("num", StandardScaler(), ["feature1", "feature2"])])
    model = Pipeline([("preprocessor", preprocessor), ("estimator", LogisticRegression())])
    model.fit(X[["feature1", "feature2"]], y)
    return model


@pytest.mark.skip(
    reason="Permutation importance has indexing issue with partial features - needs fix"
)
