...
class ValidationConfig(BaseModel):
    ...
    baseline_model: ModelSourceConfig | None = None
    # Holdout dataset
    ...
    segmentation_columns: list[str] | None = None
    # Input features
...
def create_suite_config_from_validation_config(config: ValidationConfig) -> SuiteConfig:
    ...
    paths = PathsConfig(
        ...
        plots_dir=Path("exports/plots"),
        model_export_dir=Path("models"),
        metrics_log=Path("exports/reports/model_metrics_log.csv"),
        log_dir=Path("logs"),
        train_data_path=config.holdout_data_path,
        test_data_path=config.holdout_data_path,
    )
