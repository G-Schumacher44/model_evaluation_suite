...
class ValidationDashboard:
    def __init__(self, results: dict[str, Any]):
        ...
        self.suite_config: SuiteConfig | None = results.get("config_obj")
    ...
    def _build_summary_tab(self) -> widgets.VBox:
        ...
        possible_metrics = [
            {
                "label": "Accuracy",
                "keys": ["accuracy", "final_accuracy", "test_accuracy"],
                "format": ".3f",
            },
            {
                "label": "F1-Score",
                "keys": ["f1_score", "final_f1_score", "test_f1"],
                "format": ".3f",
            },
            {
                "label": "Precision",
                "keys": ["precision", "final_precision", "test_precision"],
                "format": ".3f",
            },
            ...
            {
                "label": "R-squared",
                "keys": ["r_squared", "final_r_squared", "test_r_squared"],
                "format": ".3f",
            },
            {
                "label": "RMSE",
                "keys": ["root_mean_squared_error", "final_rmse"],
                "format": ",.2f",
            },  # ADDED
            {
                "label": "MAE",
                "keys": ["mean_absolute_error", "final_mae"],
                "format": ",.2f",
            },  # ADDED
        ]
        ...
        if not kpi_html_parts:
            return widgets.VBox(
                [
                    _build_html_card(
                        "Final Holdout Metrics", "<i>No summary metrics found in results.</i>"
                    )
                ]
            )
    ...
    def _build_plot_viewer_tab(self, title: str, sub_directory: str) -> widgets.VBox:
        ...
        if not plot_dir.is_dir() or not any(
            ...
            return widgets.VBox(
                [widgets.HTML(f"<p><em>No plot images found in '{sub_directory}'.</em></p>")]
            )
    ...
    def _build_explainer_tab(self) -> widgets.VBox:
        ...
        description = widgets.HTML(
            "<p>Enter a row index to explain its prediction on the holdout set.</p>"
        )
        ...
        title = widgets.HTML(
            '<h4 style="margin-top:0; border-bottom: 1px solid #e1e4e8; padding-bottom: 8px;">SHAP Local Explainer</h4>'
        )
    ...
    def _build_baseline_comparison_tab(self) -> widgets.VBox:
        ...
        if not baseline_metrics:
            return widgets.VBox(
                [widgets.HTML("<i>No baseline metrics available for comparison.</i>")]
            )
        ...
        for key, val in baseline_metrics.items():
            ...
            items.append(
                f"<tr><td style='padding:4px 12px;'><b>{key}</b></td><td style='padding:4px 12px;'>{value_str}</td></tr>"
            )
    ...
    def _build_dashboard(self) -> widgets.VBox:
        if not self.suite_config:
            return widgets.VBox(
                [
                    widgets.HTML(
                        "<h2>Validation Run Failed: Configuration object not found in results.</h2>"
                    )
                ]
            )
        ...
        tab_children = [
            ...
            self._build_plot_viewer_tab(
                title="Feature Distributions", sub_directory="distributions"
            ),
