...
def plot_feature_distributions_classification(
...
def plot_accuracy_confidence_interval(
    y_true: pd.Series, y_pred: pd.Series, config: SuiteConfig
) -> str:
    ...
    ax.errorbar(
        x=accuracy,
        y=0,
        xerr=[[accuracy - ci_low], [ci_upp - accuracy]],
        capsize=5,
        capthick=2,
        ecolor="blue",
        marker="s",
...
def plot_performance_by_segment(
    X: pd.DataFrame, y_true: pd.Series, y_pred: pd.Series, segment_column: str, config: SuiteConfig
) -> str:
