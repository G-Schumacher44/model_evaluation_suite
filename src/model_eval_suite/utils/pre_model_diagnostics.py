...
def run_outlier_check(
    df: pd.DataFrame, target_column: str = None, save_path: Path = None
) -> pd.DataFrame:
    """
...
def check_skewness(
    df: pd.DataFrame, target_column: str = None, skew_threshold: float = 0.75
) -> pd.DataFrame:
...
def run_vif_check(
    df: pd.DataFrame, target_column: str = None, vif_threshold: float = 5.0, save_path: Path = None
) -> pd.DataFrame:
    """
...
def run_correlation_check(
    df: pd.DataFrame, target_column: str = None, method: str = "pearson", save_path: Path = None
) -> pd.DataFrame:
    """
...
def _generate_diagnostic_html_report(
    results: dict[str, pd.DataFrame], plot_paths: dict[str, Path], save_path: Path, run_id: str
):
...
def run_pre_model_diagnostics(df: pd.DataFrame, config: dict[str, Any]) -> dict[str, pd.DataFrame]:
    ...
    diagnostics_results = {
        ...
        "skewness_summary": check_skewness(
            df, target_column, diag_config.get("skewness_threshold", 0.75)
        ),
        "outlier_summary": run_outlier_check(df, target_column, plot_paths["outlier_boxplots"]),
        "vif_scores": run_vif_check(
            df, target_column, diag_config.get("vif_threshold", 5.0), plot_paths["vif_plot"]
        ),
        "correlation_matrix": run_correlation_check(
            df, target_column, save_path=plot_paths["correlation_matrix"]
        ),
    }
