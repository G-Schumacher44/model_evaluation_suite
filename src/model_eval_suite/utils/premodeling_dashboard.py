...
class PreModelingDashboard:
    ...
    def _build_overview_tab(self) -> widgets.VBox:
        ...
        if outlier_df is not None and not outlier_df.empty:
            items.append(
                f"<li><b>{len(outlier_df)}</b> numeric features have potential outliers.</li>"
            )
        ...
        if vif_df is not None and not vif_df.empty:
            ...
            if high_vif_count > 0:
                items.append(
                    f"<li><b>{high_vif_count}</b> features have high multicollinearity (VIF > {vif_threshold}).</li>"
                )
    ...
    def _build_report_panes(self):
        ...
        dist_pane = widgets.HBox(
            [
                _build_html_card("Skewed Features", skew_df.to_html(classes="table"))
                if skew_df is not None and not skew_df.empty
                else widgets.HTML("<i>No highly skewed features found.</i>"),
                _build_html_card("Outlier Counts", outlier_df.to_html(classes="table"))
                if outlier_df is not None and not outlier_df.empty
                else widgets.HTML("<i>No outliers detected.</i>"),
            ]
        )
    ...
    def _build_plots_tab(self) -> widgets.VBox:
        ...
        if not plot_dir.is_dir() or not any(
            ...
            return widgets.VBox([widgets.HTML("<p><em>No diagnostic plot images found.</em></p>")])
