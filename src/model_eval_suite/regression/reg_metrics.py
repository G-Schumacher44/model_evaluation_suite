...
def generate_audit_alerts(results: dict, config: SuiteConfig) -> list:
    ...
    if rmse > audit_config.rmse_max_threshold:
        alerts.append(
            f"⚠️ High Error: RMSE is high ({rmse:,.2f}), indicating large prediction error."
        )
    ...
    if baseline:
        ...
        if r2 < baseline_r2 * audit_config.performance_regression_r2_threshold_factor:
            alerts.append(
                f"⚠️ Regression Performance Drop: R² ({r2:.3f}) is more than 5% lower than baseline ({baseline_r2:.3f})."
            )
