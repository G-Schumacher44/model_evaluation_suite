...
def plot_predicted_vs_actual(
    model: Pipeline, X_test: pd.DataFrame, y_test: pd.Series, config: SuiteConfig, save_dir: Path
) -> str | None:
...
def plot_learning_curve(
    model: Pipeline, X: pd.DataFrame, y: pd.Series, config: SuiteConfig, save_dir: Path
) -> str | None:
...
def plot_permutation_importance(
    ...
    try:
        ...
        final_features = np.array(model.named_steps["preprocessor"].get_feature_names_out())[
            sorted_idx
        ]
...
def plot_shap_summary(
    results_data: dict[str, Any], config: SuiteConfig, plot_type: str, save_dir: Path
) -> str | None:
    # SHAP summary or bar plot depending on plot_type
    if "shap_values" not in results_data:
        return None
...
def plot_feature_distributions(
    ...
    for col in factory_config.numeric_features:
        ...
        ax.set_title(f"'{col}' vs. Target Variable")
        ax.grid(True)
...
def generate_all_plots(
    ...
    if plot_configs.predicted_vs_actual.get("save", False):
        plot_paths["predicted_vs_actual"] = plot_predicted_vs_actual(
            model, X_test, y_test, config, eval_save_dir
        )
    ...
    if plot_configs.residuals_histogram.get("save", False):
        plot_paths["residuals_histogram"] = plot_residuals_histogram(
            model, X_test, y_test, config, eval_save_dir
        )
    ...
    if plot_configs.learning_curve.get("save", False):
        plot_paths["learning_curve"] = plot_learning_curve(
            model, X_train, y_train, config, eval_save_dir
        )
    if plot_configs.permutation_importance.get("save", False):
        plot_paths["permutation_importance"] = plot_permutation_importance(
            model, X_test, y_test, config, eval_save_dir
        )
    ...
    if "shap_values" in results and plot_configs.shap_summary.get("save", False):
        ...
        plot_paths["shap_beeswarm_plot"] = plot_shap_summary(
            results, config, "beeswarm", eval_save_dir
        )
