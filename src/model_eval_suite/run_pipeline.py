...
def main(user_config_path: str | None = None):
    """
    ...
    with (
        importlib.resources.files("model_eval_suite.config")
    ...
    with mlflow.start_run(run_name=config.run_id):
        ...
        if config.pre_model_diagnostics and config.pre_model_diagnostics.run:
            ...
            diagnostic_results.update(
                run_pre_model_diagnostics(df.copy(), config=config.model_dump())
            )
        ...
        pipeline = pipeline_factory(
            ...
            fe_config=config.modeling.feature_engineering.model_dump()
            if config.modeling.feature_engineering
            else None,
        )
        ...
        model_registry_name = (
            config.modeling.pipeline_factory.registered_name
            or config.modeling.pipeline_factory.name
        )
        ...
        if config.evaluation.run:
            if config.task_type == "classification":
                from model_eval_suite.classification.class_evaluator import (
                ...
                from model_eval_suite.regression.reg_evaluator import (
                    orchestrate_model_evaluation as orchestrate_regression_evaluation,
                )
            ...
            metrics_to_log = {
                k: v for k, v in results.get("metrics", {}).items() if isinstance(v, (int, float))
            }
            ...
            if config.notebook_mode:
                ...
                try:
                    if config.task_type == "classification":
                        from model_eval_suite.classification.class_dashboard import (
                            display_evaluation_dashboard,
                        )

                        display_evaluation_dashboard(results)
                    elif config.task_type == "regression":
                        from model_eval_suite.regression.reg_dashboard import (
                            display_regression_dashboard,
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Optional: Path to the user's override config.yaml file.",
    )
